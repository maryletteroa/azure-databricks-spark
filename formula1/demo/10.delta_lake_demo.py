...
# MAGIC %sql
# MAGIC SELECT * FROM f1_demo.drivers_merge TIMESTAMP AS OF "2021-09-24T01:57:53.000+0000";

# COMMAND ----------

df = spark.read.format("delta").option("timeStampAsOf", "2021-09-24T01:57:53.000+0000").load("/mnt/formula1dlmr/demo/drivers_merge")
...
# MAGIC %sql
# MAGIC -- for gdpr purposes, use vaccumm to remove data from history
# MAGIC -- default is remove data that are 7-days old or more, specify RETAIN
# MAGIC 
...
# MAGIC SELECT * FROM f1_demo.drivers_merge;
# MAGIC 
# MAGIC -- keep in mind to limit storing the transaction logs sice that might cost
...
# example when migrating projects
...
df.write.format("parquet").save("/mnt/formula1dlmr/demo/drivers_convert_to_delta_new")

# COMMAND ----------

# MAGIC %sql
# MAGIC CONVERT TO DELTA parquet.`/mnt/formula1dlmr/demo/drivers_convert_to_delta_new`
